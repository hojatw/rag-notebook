...
- Produce the report in the guide's format (走查方法 / 總評 / 做得好的 / 嚴重度分表 with `問題 | 方向 | 修改建議 | backlog 對應 | 狀態` / 優先順序). Update `docs/archive/2026-06-19-UX_REVIEW.md` in place: add new findings, tick `[x]` items that are now fixed, keep rows for traceability.
