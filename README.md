...
- **AI governance telemetry backend**: `llm_usage_events` records compact per-call LLM/embedding usage for retrieval, answer generation, Studio tools, summaries, follow-ups, and eval authoring/runs. `ai_safety_events` records local safety-rule findings for selected user inputs (length, invisible/control text, likely secrets, prompt-injection-style phrases) as redacted review signals. Provider token usage is preferred when available, streaming usage is requested with safe fallback, and governance metadata avoids raw prompts, source text, model output, retrieved snippets, and API keys.
...
app/templates/
  ...
  admin_eval_compare.html Run comparison page (param/metric/per-question diff of two runs).
...
tests/
  ...
  test_governance.py   LLM usage telemetry, safety-rule scanning, sanitized persistence, retry metadata, and provider/stream usage recording.
