...
**例外：需要人為動作的修正也進 MINOR。** 如果升級後維運者必須做點什麼——重新索引、
改設定鍵、重設密碼、跑資料轉換——即使內容全是修正，也提 MINOR。

因為 `0.x` 階段 MAJOR 不動，MINOR 是唯一能傳達「這一版升級要注意」的訊號，而版號是
維運者**最先看到**的東西，比 CHANGELOG 早。`0.6.0 → 0.6.1` 讀起來是「安全，直接升，
不用做什麼」；當實情是「不重新索引的話修正根本不會生效」時，那個版號等於在說謊。

判斷方式很直接：**如果你在 CHANGELOG 寫了「升級注意事項」，那就是 MINOR。**
這兩件事本來就該一起發生——需要寫升級注意事項，正是因為有人得動手。

（實例：`0.7.0` 全部是修正，沒有任何新功能，但改變了 PDF／PPTX／DOCX 的表格文字形狀
與 XLSX／CSV 的分塊切法，不 Rebuild 就完全沒有效果。照字面規則它會是 `0.6.1`，
那會讓維運者以為可以直接升。）
