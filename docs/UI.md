...
- **HTMX 局部更新**:就地更新用 `hx-target`/`hx-swap="outerHTML"`,partial 命名 `_*.html`;
  ...
  - **驗證失敗必須看得見**：HTMX 預設不交換 4xx 回應。來源比較的結果容器明確標記
    `data-swap-validation-errors="400"`，由 `app.js` 的 `htmx:beforeSwap` 只允許此容器
    顯示 HTTP 400 的 HTML 驗證片段，並保留錯誤狀態；不全站放行 4xx，不攔改 403/404/500
    或 JSON 錯誤。訊息使用 `.alert` + `role="alert"`，保留來源選擇與主題，讓使用者修正後重試。
    驗證時須在桌面與手機寬度實際送出「4 份來源 + 主題」與「11 份來源 + 無主題」，
    確認錯誤出現在視窗內且按鈕恢復可用；
    只驗證 TestClient 回應包含文字不足以覆蓋這個行為。
...
- **來源比較結果**：生成內容使用「共同點／差異／待釐清」（依來源語言使用對應詞彙），
  空段落省略。「差異」按比較面向並列來源內容；「待釐清」說明依據、不確定條件與
  待確認問題，不把不同專案、版本或未檢索到的內容直接判為矛盾。這是 LLM prompt
  的內容約定，不是 UI 固定標籤；已儲存筆記不改寫，需重新生成才會套用。
  結果最前方由程式固定產生「來源對照與證據狀態」，將 `[n]` 對應到檔名及
  「使用摘要／節錄」、「已取得主題證據（N 段）」或「未取得足夠主題證據」。
  段數只計本次實際送入比較的非空片段，不代表模型已完整涵蓋該來源。
  此對照隨 Markdown 一起存入筆記及匯出；不得只顯示在視窗而讓筆記失去對照。
  共同點逐項列明適用來源，只有部分來源共同時必須明示，不能只寫「兩份報告」。
  共同點至少需要兩份文件支持，單一文件內不同群組／產品的相似性不算跨文件共同點。
