...
- **Fix:**
  1. Make judgeability explicit: with no `expected_answer`, report `answer_quality` as `not_applicable` rather than pooling a manufactured label into `correct_rate`. **`groundedness` and `citation_correctness` stay valid without a reference** — they only compare the answer against the retrieved chunks — so this is a targeted gate, not a redesign.
