...
**目前版本的涵蓋範圍限制**（改善項目已列入路線圖；導入前請向使用者說明）：

- **來源摘要只取自文件前段。** 每份來源的 2–4 句摘要目前依文件開頭約前幾頁產生。
  以摘要為基礎的功能——notebook 簡報、未指定主題的來源比較、學習指南、FAQ、時間軸、
  摘要翻譯——因此反映的是各文件的開頭，而非全文；長篇報告若開頭沒有摘要章節，
  結果可能偏向封面、目錄與前言。需要全文細節時，請改用有引用的問答，或指定主題的來源比較
  （會先從各來源檢索證據）。
- **會議記錄只讀取逐字稿前段。** 目前最多讀取約 16,000 字，較長的會議後段（通常是決議所在）
  不會被納入，且畫面上尚未提示涵蓋範圍。
...
- 產品、法規、醫藥、技術文件問答。
- 客戶專案背景整理。
- 會議逐字稿轉會議記錄（長會議目前只涵蓋前段，見上方「涵蓋範圍限制」）。
- 內部 FAQ 與知識庫初稿建立。
- 多份文件的共同點、差異與待釐清事項整理。
- 模型/檢索品質的內部評估與調校。
