...
- **The bound is characters, never tokens.** `target_chars` is the only limit this stage enforces, and nothing downstream measures real tokens before the embedding call, so a chunk inside the char cap can still exceed the model's input window when the text is dense. Measured against the real multilingual-e5 tokenizer at the 800-char Latin cap: English prose ~180 tokens, Traditional Chinese ~465, but table rules (`├─┼─┤`) 564, full-width punctuation 634 and mis-decoded text ~780. What made this concrete was a table renderer emitting a separator per *empty* cell — a sparse FDA drug-label table became a wall of `| | | |`, each pipe costing two tokens (`▁` then `|`) and carrying nothing — for 533 tokens in 773 characters. vLLM answers an over-long embedding input with HTTP 400 rather than truncating, so one such chunk fails the whole batch and the whole source. The renderers now collapse blank-cell runs (`_collapse_row_cells`), and `estimate_embedding_tokens` prices per character class so the `chunk_over_token_budget` warning actually fires; neither is a hard guarantee. Check a specific file with `python -m tests.inspect_file_tokens <path>`.
...
**Input window:** `embed_texts` trims any text estimated over the model's input window before sending, and logs `embedding_input_truncated` when it does. The window comes from `/settings` → *Test embedding model*, which overshoots the endpoint once and reads the limit out of its rejection (`maximum context length is N tokens`); `llm.embedding_window_budget` reads that back, falling back to `[diagnostics].embedding_token_budget` (512, e5's) whenever no probe matches the current connection. **Re-test after changing the embedding model** — until you do, a wider model keeps being trimmed at the old figure, which costs recall and says nothing. This matters most on the **query** side: chunks are already packed to this budget by the chunker, but a pasted question had no limit anywhere, and e5 answers an over-window input with HTTP 400 rather than truncating — so one long question failed the whole request and the user got no answer. Trimming keeps the leading part, which carries most of the retrieval signal. A trim logged with `role=passage` means the chunker produced an over-budget chunk — that is a bug in the chunker, not a tuning issue. The config value still matters: it is what an unprobed deployment uses, and it also sizes spreadsheet chunks, so changing it re-shapes stored chunks and needs a re-index (the probed window does not — it only caps what is sent).
