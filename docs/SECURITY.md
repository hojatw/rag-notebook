...
3. **Someone other than the author has completed an upgrade using only the documentation — attempted 2026-09-09, not yet satisfied.** A colleague upgraded a customer deployment to `0.7.0` (checkout → build image → `docker compose up`) without step-by-step help, which is the right shape of evidence. It does not count yet because the documentation did not carry them through: the upgrade left `data/chroma` with a corrupt HNSW segment, the app kept answering from the degraded SQLite fallback instead of failing, and recovering needed the judgement that the fault was in the derived vector index rather than in a SQLite migration — a step no document described. The gaps found were then closed: [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) §1 now carries the symptoms, log strings and recovery, both READMEs tell operators to stop the old processes before starting the new ones and to check for `retrieve_vector_failed` afterwards, and `ROADMAP.md` `O3` tracks making that failure admin-visible. **This condition is satisfied when the next upgrade by someone other than the author completes on the documentation alone** — record the date, version and any gap found here, the way this attempt is recorded.
