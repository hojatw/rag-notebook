...
- **Foundation landed (2026-06-19):** `app/i18n.py` catalog + `t()` Jinja global + `window.I18N` (base.html) + `[ui].language` config (`NOTEBOOKLM_UI_LANGUAGE`, default `zh-TW`, en deferred). Migrated so far: primary nav, the **audit page** (fully), the **Studio/chat surfaces** (chat empty-state, Studio tools + suggestions, tool config panels, app.js answer-flow strings, chat abstain, Markdown export headings), **user-facing server errors** (`friendly_error_message` mapper + action labels, the per-flow inline errors, SSE status texts), and the **upload widget + settings provider hints** (`tr()` now supports `{name}` formatting; `app.js` has no hardcoded Chinese left — all via `tr()`/`window.I18N`). The Phase 1 user surfaces are done. The Eval workbench is in progress (hybrid naming — `評測` for the activity, `Eval Set`/`Eval run` kept as data terms, `Retrieval`→`檢索`, `Profile`/Recall/MRR preserved): the **nav label, eval tabs, and the `/admin/evals` landing page** are migrated (incl. the run-status label map). Remaining: the Retrieval-Profiles page, eval-set detail / runs / compare / help pages and their partials, settings labels (L4), and global aria-labels (L3) — tracked in [`docs/UX_REVIEW.md`](UX_REVIEW.md) (i18n Phase 2b-ii).
