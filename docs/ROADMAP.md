...
#### [ ] O5 · Probe the endpoint's JSON and embedding-window capabilities
- **Issue:** two settings are guessed rather than known. (a) Whether the chat endpoint supports `response_format: {"type": "json_schema"}` — it would end malformed-JSON salvage entirely (TROUBLESHOOTING §5) instead of recovering from it. (b) The embedding model's input window, currently the fixed `[diagnostics].embedding_token_budget` (512, e5's). A deployment that moves to a wider model (OpenAI `text-embedding-3` is 8191) silently keeps trimming queries at 512 unless an operator remembers to raise it (§4).
- **Target model:** the same shape `O1b` already established for chat sampling — `/settings` probes once, stores the answer in `llm_settings.diagnostics_json`, and the runtime reads it back through a resolver. Never infer either from the model name: `AGENTS.md` forbids it and the name-matching it forbids breaks on every release.
- **Window probe:** binary-search the accepted input length against the live endpoint (~10–14 requests, only on *Test embedding model*). Handle both rejection shapes seen in production — a token limit (`value=513`) and a character limit (`more than 8192 characters`) — and fall back to the config value when the probe cannot conclude, so an unprobed deployment keeps working.
- **Guardrail on the window:** `embedding_token_budget` also sizes spreadsheet chunks, so a probed value that differs from the stored one means existing sources need re-indexing before they benefit. Surface that, do not silently re-shape chunks.
- **Related:** makes the salvage in §5 a fallback rather than the primary defence, and removes the manual step §4 currently depends on.
