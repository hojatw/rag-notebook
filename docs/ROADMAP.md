...
- **Foundation landed (2026-06-19):** `app/i18n.py` catalog + `t()` Jinja global + `window.I18N` (base.html) + `[ui].language` config (`NOTEBOOKLM_UI_LANGUAGE`, default `zh-TW`, en deferred). Migrated so far: primary nav, the **audit page** (fully), the **Studio/chat surfaces** (chat empty-state, Studio tools + suggestions, tool config panels, app.js answer-flow strings, chat abstain, Markdown export headings), **user-facing server errors** (`friendly_error_message` mapper + action labels, the per-flow inline errors, SSE status texts), and the **upload widget + settings provider hints** (`tr()` now supports `{name}` formatting; `app.js` has no hardcoded Chinese left — all via `tr()`/`window.I18N`). **Complete (2026-06-19).** Every user-facing and admin surface now resolves copy through the catalog: Phase 1 (nav, chat, Studio tools, server errors, upload widget + settings hints) and Phase 2 — the audit page and the **whole Eval workbench** (landing, Profiles, eval-set detail / runs / compare / help + partials), plus status label maps (source / run / eval-result / sensitivity), global aria-labels (L3), and the settings bilingual label (L4). Hybrid eval naming: `評測` for the activity; `Eval Set`/`Eval run`/`Run`/`Baseline`/`Candidate` and Recall/MRR/Profile/chunk/embedding/RAG kept as domain terms. Documented exceptions in [`docs/UX_REVIEW.md`](UX_REVIEW.md): the deepest already-zh eval prose stays inline (not catalog-routed; display language unaffected), and `PROFILE_PARAM_LABELS` technical param names are preserved. Remaining i18n is **U15b only** (populate an `en` locale + optional per-user toggle).
