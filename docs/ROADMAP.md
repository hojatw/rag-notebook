...
- **Foundation landed (2026-06-19):** `app/i18n.py` catalog + `t()` Jinja global + `window.I18N` (base.html) + `[ui].language` config (`NOTEBOOKLM_UI_LANGUAGE`, default `zh-TW`, en deferred). Migrated so far: primary nav, the **audit page** (fully), the **Studio/chat surfaces** (chat empty-state, Studio tools + suggestions, tool config panels, app.js answer-flow strings, chat abstain, Markdown export headings), and **user-facing server errors** (`friendly_error_message` mapper + action labels, the per-flow inline errors, SSE status texts). Remaining: upload-file JS strings (need JS-side formatting), settings provider hints, and the Eval workbench cluster — tracked in [`docs/UX_REVIEW.md`](UX_REVIEW.md) (i18n Phase 1c / 2b).
