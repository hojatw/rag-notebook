...
- **Implementation plan:** [`O0_DIMENSION_RESET_PLAN.md`](O0_DIMENSION_RESET_PLAN.md) — phased breakdown, test matrix, and the design decisions. **D1 (refuse while a job runs), D2 (stale-source status), and D3 (flow lives on `/admin/index`) are ruled and implemented**; D4 (script retirement) is deferred to Phase D.
- **Phase A + B + C landed:** `reset_collection()` replaces the collection object and bumps `vector_index_state.generation`, which invalidates every other process's cached handle; `app/index_migration.py` classifies sources by stored-vector width; `sync_from_sqlite` skips dimension-mismatched chunks (reporting `skipped_dimension`), and old-width sources move to `stale_embedding` — excluded from every `status = 'indexed'` query and recovered through Reindex. `/admin/index` now carries the migration itself: the target width comes from the last successful `/settings` embedding test (never typed in), the page shows a dry-run preview, the operator types the dimension back to confirm, a running ingest job refuses the whole thing, and the migration holds a lock that pauses `claim_next_job()` for its duration — audited as `index_dimension_migrated`. Remaining: split-worker coverage and removing the temporary warnings (Phase D).
- **Permanent-fix acceptance criteria:**
  1. **[done]** Clear used for a dimension migration deletes/recreates the collection schema, not only its records.
  2. **[done]** App and dedicated worker cannot keep stale cached collection handles after reset (generation counter); concurrent ingest is blocked (D1: a running job refuses the migration, and the migration lock pauses further claims).
  3. **[done]** Startup sync cannot silently reinsert old-dimension SQLite embeddings and re-lock the new collection; affected sources receive an explicit Reindex state/path.
