...
- **Phased (tick as done):**
  ...
  - [ ] E1g — **Warn on the compare page when two runs were served by different chat models.** The data and the diff already exist: `compare_runs_context` (`app/evals.py`) diffs every `LLM_SNAPSHOT_LABELS` key between the two runs' `llm_snapshot_json` and flags each changed row, and `admin_eval_compare.html` renders them with `diff-changed`. What is missing is **placement**: that table is its own section *below* the metric diff, so an admin reading Recall/MRR/judge deltas at the top can attribute a difference to a profile change when the chat model also changed. Chat runs on two switchable machines and switching is normal operation ([`DEPLOYMENT_CONTEXT.md`](DEPLOYMENT_CONTEXT.md), 2026-09-17 addendum), so this is a routine case, not an edge one. **Scope, deliberately small:** when `chat_model` (and arguably `embedding_model`) differs between the two runs, show a caveat above the metric tables saying the runs are not directly comparable and pointing down at the LLM diff — no new schema, no new capture, no blocking. New copy goes through `app/i18n.py` per [`I18N.md`](I18N.md). **Test:** two runs differing only in the `chat_model` of their stored snapshot must render the caveat, and two runs sharing it must not — observed failing first, per AGENTS.md. **Why it matters:** `QUALITY.md` `Q1-3`, `Q1-5` and `Q1-6` all now carry a "check the snapshot before comparing" discipline; this makes the tool enforce what those items currently ask a person to remember. **Raised:** 2026-09-19, from the re-evaluation of the "serving side is fixed" premise.
