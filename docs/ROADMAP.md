...
**Current target-deployment constraint:** the known customer inference side is borrowed — and **two switchable chat hosts, not one fixed host** (Gemma 4 31B on the newer machine, gpt-oss-120b on the older one; see the 2026-09-17 addendum in [`DEPLOYMENT_CONTEXT.md`](DEPLOYMENT_CONTEXT.md)) + multilingual-e5-large embeddings — **chat + embedding only unless the active chat endpoint passes the image-understanding probe**). Features needing only chat completions are cheap; new extraction paths (web, PPTX, spreadsheets, OCR) are app-side work; new model capabilities (vision, speech) must be verified against the active customer endpoint first. `O1` adds admin capability probes so this assumption can be tested per deployment before enabling vision-dependent work.
...
- **Fix:** **Done.** Asking posts to a streaming endpoint from the enhanced UI. Retrieval/generation status arrives incrementally; after E2, provider answer text is deliberately buffered and classified, then emitted as one final answer event before the saved message swaps into the normal Markdown/citation/follow-up rendering path. This prevents a late/truncated structural abstain marker from leaking earlier text. Same item as [`PERFORMANCE.md` P3-1](PERFORMANCE.md).
...
- **Phases 1–2 done** — tools tile grid, slim briefing strip, suggestions relocated to chat, and typed outputs shelf. Phase 3 is optional and remains unscheduled.
- **Original issue:** The Studio (right pane) stacked one always-on card per generator and mixed **generators (actions)** with **outputs (artifacts)**, so every new AI feature added another card and made the column longer.
- **Implemented model:** separate **tools** from **outputs**, so adding a feature = +1 tile, not +1 always-on card.
...
- **Original scope:** U4 had hardcoded zh-TW strings directly in templates / `app.js` / server messages. The scoped first pass was a message catalog, template/JS helpers, and a config-driven deployment locale covering high-churn surfaces; the implementation below is the completed result.
- **Complete (2026-06-19).** Dependency-free catalog (`app/i18n.py`) + `t()` Jinja global + `window.I18N` (base.html) + `tr()` in `app.js` + `[ui].language` config (`NOTEBOOKLM_UI_LANGUAGE`, default `zh-TW`). Shared/high-churn copy was migrated across nav, chat/Studio states, server errors, upload/settings hints, audit, and Eval Workbench chrome, plus status label maps, global aria-labels (L3), and the settings bilingual label (L4). This was a **foundation, not a complete extraction**: legacy inline zh-TW template prose remains and must be inventoried/extracted before a second locale can claim full coverage. Hybrid eval naming keeps `Eval Set`/`Eval run`/`Run`/`Baseline`/`Candidate` and Recall/MRR/Profile/chunk/embedding/RAG as domain terms. Only `zh-TW` is populated. **Full reference + current exceptions: [`docs/I18N.md`](I18N.md).** Phase-by-phase landing log is in [`docs/archive/2026-06-19-UX_REVIEW.md`](archive/2026-06-19-UX_REVIEW.md).
...
- Work required (full design in [`docs/I18N.md`](I18N.md) "Planned" section):
  ...
  2. Inventory/extract the remaining inline zh-TW template copy, then make the
     **status label maps locale-aware** (`SENSITIVITY_LABELS` / `RUN_STATUS_LABELS` /
     `EVAL_RESULT_STATUS_LABELS` / `source_status_labels` are single-locale today).
...
#### [x] U17 · Meaningful topic-focused source comparison
- **Historical issue:** the compare tool used to expose a **聚焦重點 (focus)** free-text input, but comparison still used each source's thin 2–4-sentence summary. The topic changed the prompt without changing the evidence, so output barely changed and the UI field was removed.
- **Implemented behavior:** the field is restored as an optional **比較主題**. A blank topic preserves summary comparison. A non-empty topic runs the existing retrieval pipeline sequentially once per authorised source with `source_ids=[that source]`, then compares the retrieved chunks instead of summaries. The active low-confidence threshold marks a source as having no sufficiently relevant topic evidence; the compare prompt must report that absence rather than infer a position.
- **Output semantics:** both modes request **共同點 / 差異 / 待釐清**, omitting empty sections. Compare explicit terms by dimension and cite source evidence; do not treat different projects/versions or missing evidence as contradictions. Unknown relationships and possible inconsistencies require qualified explanations and confirmation questions, not verdicts. These are prompt-level guardrails, not automatic project/version verification or exhaustive document diffing; see `RETRIEVAL.md`.
- **Source traceability:** an application-owned legend maps the model's `[n]` source IDs to filenames and input evidence status/counts, retained in saved notes and exports. Prompt rules require each bullet to identify supporting sources and label subset-only shared points explicitly; input coverage is not a promise of complete model-output coverage. A real route → compare prompt → save → export test pins the numbering contract.
- **Bounded work:** topic mode accepts 2–3 authorised sources. Four or more are rejected before retrieval because each source adds query rewrite, embedding, vector/keyword search, and rerank calls. Summary mode accepts 2–10 authorised sources in a single comparison call; eleven or more are rejected before generation. The summary cap limits aggregate input and report complexity, but is not a guarantee that every endpoint's context window is sufficient. These fixed product limits are intentionally not configurable yet; reconsider them based on customer feedback.
- **Validation boundary:** representative customer documents can measure product usefulness later, but they are not an implementation prerequisite. Route/prompt tests pin per-source scoping, authorization, summary fallback, low-confidence handling, and the three-source topic / ten-source summary caps. Browser verification must also submit an over-limit selection: the comparison result pane opts into displaying HTTP 400 HTML validation fragments, because HTMX otherwise leaves them invisible. The HTTP error status and the user's selections/topic are preserved.
...
#### [x] E1 · In-deployment eval workbench (private customer data stays in place)
- **Original issue:** Several quality/performance items were blocked on representative customer-style eval data (`QUALITY.md` Q0-2 / Q1-1 / Q1-2 / Q1-4, `PERFORMANCE.md` P3-2). Customer source data may be unable to leave the deployment, so the app needed an admin-only way to build and run evals **inside** the customer's environment.
- **Target model (implemented):** an admin creates eval sets from already-indexed DB data, runs retrieval/profile experiments with visible progress and optional answer/citation judging, compares every run historically, applies/rolls back approved runtime-safe parameters, and exports sanitized settings/reports for the implementation team.
- **Guardrails:**
  - Generated candidate questions are suggestions only; admins must review/approve/edit before they become ground truth.
  - Each run stores immutable retrieval-profile and E2 domain snapshots, aggregate metrics, and per-question results. Do not claim an eval-set/LLM-settings snapshot unless those fields are explicitly persisted in a future schema change.
  - Runtime-safe parameters can be applied immediately; index-affecting parameters (chunk sizes, overlap, embedding model/prefix/dimension) must be shown with strong Reindex warnings and should not be silently applied. Embedding-dimension changes use the completed `/admin/index` migration flow (O0), never Clear/Rebuild.
...
- **Phased (tick as done):**
  ...
  - [x] E1d — **Done.** Export + audit foundation landed. Retrieval profiles can be exported as sanitized JSON; eval runs support sanitized JSON (no questions/evidence/retrieved snippets/domain text/answers/judge rationale) and a CSRF-protected confirmed POST for full internal JSON. Full reports can include questions, evidence, diagnostics, retrieved snippets, generated answers, judge details, and the frozen domain snapshot. A durable `audit_events` table and `/admin/audit` viewer record exports plus high-risk admin actions without copying those payloads into audit metadata.
  ...
  - [x] E1e-2 — **Done.** Answer-quality and citation judging (route A, comparative/structured). Opt-in per run (`judge_enabled`, default off, ~2× LLM cost): each item runs the deterministic score gate, then — unless it abstains there — generates an answer; generation may also abstain through the structural marker. Answered items are LLM-judged for answer quality, groundedness, and citation correctness, while abstain correctness is deterministic. Answer-quality correctness is discarded as `not_applicable` when no reference answer exists. The former `substring_hit_rate` answer anchor was withdrawn because it incorrectly reused retrieval-evidence substrings. Judge metrics are shown **separately** from Recall/MRR and labelled a reference signal, not ground truth. A single item's generate/judge failure is isolated and never fails the run; `judge_enabled=0` regresses to retrieval-only behaviour. Generated answers/judge details appear only in audited full internal exports. **Implementation record:** [`E1E2_ANSWER_JUDGING_PLAN.md`](archive/E1E2_ANSWER_JUDGING_PLAN.md); current caveats: [`QUALITY.md`](QUALITY.md) Q1-6.
  - [x] E1f — **Done.** Eval tuning guide landed as `/admin/evals/help` and as a first-class tab in the Eval workbench. It converts the internal tuning PDF/discussion into HTML covering: when to tune parameters vs fix Eval items, symptom -> likely cause -> parameter guidance, profile experiment workflow, starter profiles, non-runtime-safe changes that require reindex, and the role of domain hints / answer policy. The PDF remains optional/shareable, but the product source of truth is now HTML so labels stay aligned with the live profile UI.
  - [ ] E1g — **Warn on the compare page when two runs were served by different chat models.** The data and the diff already exist: `compare_runs_context` (`app/evals.py`) diffs every `LLM_SNAPSHOT_LABELS` key between the two runs' `llm_snapshot_json` and flags each changed row, and `admin_eval_compare.html` renders them with `diff-changed`. What is missing is **prominence**. (An earlier draft of this item said the table sits *below* the metric diff; it does not — it is the second of five sections, above the metric tables. The gap is not order.) A changed chat model renders as one highlighted row in a seven-row table, which is visually identical to a changed row in the **profile parameter** table directly above it — and there a change is the *expected* thing, since it is what the comparison is for. So the one difference that invalidates the comparison looks exactly like the difference the page exists to show. Chat runs on two switchable machines and switching is normal operation ([`DEPLOYMENT_CONTEXT.md`](DEPLOYMENT_CONTEXT.md), 2026-09-17 addendum), so this is a routine case, not an edge one. **Scope, deliberately small:** when `chat_model` or `embedding_model` differs between the two runs, show a caveat in the page header — before any table — naming which one changed and saying the metric diff below cannot be attributed to the profile change alone. No new schema, no new capture, no blocking, no new CSS (`.warn` already exists). New copy goes through `app/i18n.py` per [`I18N.md`](I18N.md). **Test:** two runs differing only in the `chat_model` of their stored snapshot must render the caveat, and two runs sharing it must not — observed failing first, per AGENTS.md. **Why it matters:** `QUALITY.md` `Q1-3`, `Q1-5` and `Q1-6` all now carry a "check the snapshot before comparing" discipline; this makes the tool enforce what those items currently ask a person to remember. **Raised:** 2026-09-19, from the re-evaluation of the "serving side is fixed" premise.
- **Open refinement:** `E1g` above is the only unticked phase — a presentation fix on the compare page, not missing capability; `E1` stays **done**.
- **Recommended next implementation round:** E2's product capability is implemented; its customer-quality validation remains under Q1-5 and is blocked on Q1-3's customer-approved, in-deployment Eval Set workflow. Further audit expansion should wait for customer requirements, e.g. explicit read-access audit for source preview/result viewing. Applying index-affecting parameters is now unblocked — O0 provides the safe reset/Reindex workflow — but still needs its own UX before profiles may change chunking or embedding settings.

#### [x] E2 · Notebook domain hints and answer policy — product capability complete
...
- **Default spreadsheet-aggregation policy (transition guard until `A6d`):** the application-level guard states that spreadsheet sources support row lookup and semantic search but not reliable counting/summing/ranking — aggregate questions over sheet sources get an explicit caveat instead of a confidently computed number. It has higher priority than owner-authored policy and is removed only when `A6d` supplies reliable aggregation.
- **Guardrails:**
...
- **Phased:**
  - [x] E2a — Schema + notebook admin/editor UI for domain hints and answer policy. Owner-only CRUD, bounded forms, audit-safe metadata, serialized revisions, and case-insensitive DB uniqueness are implemented.
  - [x] E2b — Feed domain hints into query rewrite / retrieval expansion with explicit limits. No re-index or extra LLM call is used; live and frozen-run limits are covered independently.
  - [x] E2c — Feed answer policy into final answer prompting without treating it as source evidence. Policy/notes remain bounded user-role JSON; the structural `[[RAG_ABSTAIN]]` marker is classified from the bounded full provider completion before SSE/persistence/export and rendered through `chat.abstain`.
    - **Carried in from E1e-2 (deliberately deferred, 2026-07-25):** replace the literal refusal sentence in `SYSTEM_PROMPT` with a **structural refusal marker** the app renders as localized copy (`i18n.t("chat.abstain")`), in both the streaming and non-streaming answer paths.
      - **Implemented with E2c:** both answer paths now use the structural marker and parse it before output/persistence; the localized refusal contract is covered by focused tests. The existing low-confidence score gate remains a separate, localized app-side refusal.
  - [x] E2d — Eval Workbench comparison path for "with hints" vs "without hints" runs. Independent flags, policy+judge prerequisite, immutable/canonical run snapshots, old-run fallback, and the baseline/hints/policy/combined mode matrix are implemented. Customer judged-run evidence remains a Q1-3/Q1-5 activity, not missing product code.
...
- **Phase 1 — low-cost in-app foundation (no new gateway required):**
  ...
  - [x] G1b — Normalize provider `usage` responses when present; retain `is_estimated` for char/token estimates so reports do not imply billing precision that the endpoint did not provide. **Backend complete:** OpenAI-compatible/Azure-style `prompt_tokens` / `completion_tokens` / `total_tokens`, common `input_tokens` / `output_tokens`, camelCase/gateway token-count fields, and nested `usage` / `token_usage` / `tokens` shapes normalize into `llm_usage_events`. Streaming chat requests ask for provider usage and safely retry without `stream_options` if the endpoint rejects it; missing usage falls back to CJK-aware estimates using the configured CJK/non-CJK ratios. Partial provider shapes are marked `is_estimated=1` whenever a prompt/completion component must be synthesized; prompt + total or completion + total is completed exactly by subtraction.
  - [~] G1c — Add `ai_safety_events` plus a first local rules engine: input length limits, invisible/control text checks, obvious secret patterns, simple prompt-injection phrases, and deployment-specific deny/allow lists. Record redacted summary/hash, not raw sensitive content. **Backend MVP landed:** schema + sanitized recorder + local rules (`local.rules.v2`, English plus Traditional/Simplified Chinese prompt-injection patterns) now record findings for chat questions, streaming chat questions, compare focus text, eval authoring target language, and manual eval questions. The MVP records `warn` / `block_candidate` review signals without blocking user workflows. Remaining full-scope work: deployment-specific deny/allow configuration, broader surfaces/output scanning, and dashboard/reporting in G1d/G1e.
...
#### [x] O0 · Reset Chroma collection dimension safely
- **Was a live P0:** `/admin/index` Clear called `collection.delete(ids=...)`. Chroma keeps the collection schema after the last vector is deleted, so an empty collection stayed locked to (for example) 1024 dimensions. `probe_index_dimension()` saw no stored embedding and reported `None`; settings then accepted a 1536-dimensional model, and the first query/upsert failed with `Collection expecting embedding with dimension of 1024, got 1536`. Repeated Clear/Rebuild could not repair it.
- **Fixed.** `reset_collection()` replaces the collection object rather than emptying it; a SQLite `vector_index_state.generation` counter invalidates every other process's cached handle; the migration holds a lock that pauses `claim_next_job()` and refuses to start while an ingest job is running; `sync_from_sqlite` skips chunks whose width disagrees with the target; sources whose vectors cannot be reused move to `stale_embedding` (excluded from search and sync, recovered by Reindex). The operator flow is **更換 embedding 維度** on `/admin/index`, with the target width taken from the last successful `/settings` embedding test and a typed confirmation. Audited as `index_dimension_migrated`.
- **Acceptance criteria — all met:**
  1. **[done]** A dimension migration deletes/recreates the collection schema, not only its records.
  2. **[done]** App and dedicated worker cannot keep stale cached collection handles after reset (generation counter); concurrent ingest is blocked (a running job refuses the migration; the lock pauses further claims).
  3. **[done]** Startup sync cannot silently reinsert old-dimension SQLite embeddings and re-lock the new collection; affected sources get the explicit `stale_embedding` → Reindex path.
  4. **[done]** `384 → migrate → 1536` is covered in both operating models — inline worker, and split worker via a second independently-loaded `vector_store` with its own module caches (`tests/test_index_migration.py`). Both split-worker tests were verified to fail when the generation check is neutered.
...
- **Evidence this matters:** a customer deployment ran in that degraded state for about **two hours** on 2026-09-09 with nobody noticing (the persisted HNSW segment was corrupt — see [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) §1). Users experienced it as "the answers got worse", which is exactly the failure mode this project treats as the worst kind. It then happened **again and for two days** on 2026-09-10 from an unrelated cause (§3): of the 13 questions asked in that window only 3 reached the vector index, and one user asked 7 times without ever getting a vector-backed answer. Two incidents, two root causes, the same blind spot — which is the argument for fixing the *visibility*, not just each cause.
- **Log side landed (2026-09-17):** `retrieve_completed` now always carries `mode=` (`chroma` / `sqlite_fallback` / `preloaded_rows`), `retrieve_vector_failed` carries `consecutive_failures=` and escalates WARNING → ERROR at 3 in a row, and `retrieval.vector_health()` exposes consecutive failures plus last success/failure timestamps. That makes the state alertable, but still only for someone reading logs.
- **Smallest useful fix (remaining):** surface `vector_health()` as a health row on `/admin/index` or an admin banner, cleared by the next successful query. A per-message hint in the retrieval debug pane ("vector search unavailable — degraded results") is a cheap second step.
...
- **Related:** `O2` (worker logs) covers a different blind spot; both are operational observability. `O4` removes one of the two causes at the source.
...
#### [ ] O6 · Bound what reaches the answer prompt
- **Issue:** the answer path assembles history + the retrieved chunks + the question, and nothing bounds the total. The chat-window guard landed for **query rewrite** only; the answer path got the `build_chat_request` backstop warning (`chat_input_over_window`) and no active protection. Its input is the larger of the two, so it hits the window first.
- **Why it was left out:** what is safe to cut differs. In rewrite, history is the droppable part and the question is the payload. In an answer, the **retrieved chunks are the payload** — dropping them produces a confident answer grounded in less evidence, which is worse than a refusal and looks identical to a good one. Cutting history first is right in both, but past that the answer path needs its own rule.
- **Target model:** reuse `chat_window_budget` (0 = unmeasured = no bound, same as rewrite). Drop history oldest-first; then, rather than truncating chunk text, **reduce the chunk count** and say so in the retrieval debug badge, so a thinner answer is visible rather than silent. Never trim a chunk mid-sentence: a half-chunk still gets cited with `[N]` and the citation then points at text the answer did not actually use.
- **Guardrail:** an answer built on fewer chunks than retrieval selected must be distinguishable from one that used them all — this is the same "degraded but looks healthy" shape as `O3`.
- **Related:** the rewrite-side guard and the window probe it reads; `O3` (making degradation visible).
...
- **Mitigation landed in `v0.9.0`:** `vector_index_state.write_seq` is bumped on every upsert/delete/clear, and `collection()` clears chromadb's cached `System` and reopens the client when it moved. This is correct but reactive: it pays a client reopen on the first read after every foreign write, so a busy ingest makes the query path reopen repeatedly.
- **Target model:** run Chroma in server mode (`chroma run`) and give both containers an `HttpClient`. The server owns the index and serializes writes, so no client holds one at all and the `write_seq` dance becomes unnecessary (keep `generation` — it is still the O0 migration's barrier).
- **Cost to weigh:** one more service in a deployment whose defining constraint is "single, often borrowed, machine" ([`DEPLOYMENT_CONTEXT.md`](DEPLOYMENT_CONTEXT.md)). Decide against measured reopen frequency from `vector_collection_handle_refreshed` before committing.
- **Guardrail:** whatever replaces it must keep `tests/test_vector_store.py::test_query_sees_records_written_by_another_process` green — with a real subprocess, since an in-process module reload shares the very cache that breaks.
...
- **Phase 1 — diagnostics before profile management:**
  ...
  - [x] O1b — **Done.** The settings diagnostics section probes streaming support, provider usage reporting, JSON-following sanity, request sampling/max-token shape, positively supported `reasoning_effort` values, and optional image understanding. Runtime request adaptation is gated by the active settings fingerprint and never inferred from model names. Admins can choose automatic semantic mapping, provider default (omit effort), or a fixed effort value; fixed mode cannot be saved until the exact selected value is positively probed. The image-understanding checkbox is off by default and sends a built-in 64×64 solid-colour test image only when explicitly enabled. Accepted-but-ambiguous results are `inconclusive`, endpoint/request rejection is `failed`, and only `succeeded` may later enable A9.
- **Phase 2 — multiple profiles + safe activation:**
  ...
  - [ ] O1d — Add safe profile activation rules. Chat-only changes can activate directly after a successful chat test. Embedding-affecting changes (model/base URL/prefix/dimension) must be blocked or strongly gated when the existing Chroma index dimension/config is incompatible. Point admins at the `/admin/index` dimension-migration flow (O0), never Clear/Rebuild.
...
- **Done.** `sources.diagnostics_json` is written by `process_source` on every (re)ingest and replaced, never appended. `extract_sections` now returns an `ExtractionResult` (sections + which extractor path ran + notes) so extractor-internal facts — above all a PDF falling back from pdfplumber to plain pypdf — are recordable; `collect_ingest_diagnostics` derives the counts, `_section_kind` breakdown, and warnings from data the pipeline already had. Four warnings, each stating the consequence: **low_text** (scanned/image-only — the A8 OCR signal), **pdf_structure_fallback** (citations degrade to page level), **chunk_over_token_budget** (silent embedding truncation — see below), **empty_sections**. Failures record `failed_stage` (`extract`/`chunk`/`embed`/`store`) so "the file had no text" reads differently from "the endpoint was down", and **failed sources can now open the preview drawer** — previously the button was disabled, leaving their diagnostics unreachable. Thresholds live in `[diagnostics]` (`app/config.py`); they only affect display and never require re-indexing — with the single exception of the `tokens_per_*` costs, which also size spreadsheet chunks (see below).
- **`chunk_over_token_budget` is an estimate, not a measurement** (`estimate_embedding_tokens`): tokens are priced per character *class* — ordinary words cheaply, CJK at ~0.75/char, and digits, symbols and identifiers at ~1/char — plus a token for the space before anything that is not a word start. It exists to make [`QUALITY.md` Q0-5](QUALITY.md) observable per-source in a real deployment without shipping a tokenizer; ground truth stays `tests/inspect_e5_chunk_tokens.py` (indexed chunks) and `tests/inspect_file_tokens.py` (one file, offline, including files that failed to index). It deliberately leans towards over-estimating, measured at ~1% false warnings and no misses on a 766-chunk real corpus. **Note the one non-display effect:** the same function is the budget the spreadsheet row packer sizes chunks against, so changing `[diagnostics].tokens_per_*` changes stored chunk shape for XLSX/CSV and requires re-indexing them.
...
- **Reassessed 2026-08-24:** `A6b` Phase 1 and `A6c` have landed, so the original format prerequisite is satisfied and aggregate warning data can now be meaningful. This remains unscheduled because no operator has yet asked for a deployment-wide view; the per-source owner-only diagnostics remain the current troubleshooting path.
- **Restart condition:** prioritize when an operator asks "is ingestion healthy across this deployment?" and the per-source drawer cannot answer it. If a future need requires per-source admin access to another user's extracted text, that is a **separate privacy decision** — it needs an explicit policy call plus an audit event, not a quiet extension of this item.

#### [ ] A12 · Document structure extraction (heading tree)
- **What:** detect a document's heading hierarchy at ingest and carry it as an **ancestor path** (`heading_path: list[str]`) on every chunk, plus a page range per node. Cross-format like `A6a` — this is not a new file format, it is a signal every existing extractor can emit.
- **Why:** today the structural signal is **zero**. `_extract_docx` flattens the whole body into one `"document"` section and never reads `paragraph.style.name`; `_extract_html` runs `soup.get_text()` so `h1`–`h6` become plain lines; the PDF path emits positional labels (`page N paragraph K`) with no heading detection. `chunk_sections` then *deliberately* packs sentences **across** section boundaries, and `_section_kind` only blocks merges across a *register* change (table/header/footer/footnote), never a *topic* change — so a chunk can straddle "3.2 研究限制" and "4. 結論". For hundreds-of-page research reports this is the most-cited structural weakness in the pipeline.
- **Payoff, in the order it arrives:** (1) chunk boundaries stop straddling sections; (2) the ancestor path prefixed into the embedded text gives contextual retrieval (cheap, well-established); (3) citations read `3.2 研究限制 (p.47)` instead of `page 47 paragraph 3`, which serves the traceability requirement directly; (4) it unlocks [`QUALITY.md`](QUALITY.md) `Q1-7`. **Note that (1)–(3) need only a heading *path*, not a traversable tree.**
- **Design precedent:** the ancestor-path shape is taken from a sibling project (AITM), which carries `heading_path` on every unit and never materialises a tree object — the tree stays implicit and recoverable from the paths. That maps almost 1:1 onto today's `(location, text)` tuples and needs **no new table**: the path fits in the existing chunk row and Chroma metadata.
- **Measured signal availability (2026-08-22, 13 real PDFs from a dev corpus — read-only sampling, *directional, not statistical*: not customer data, not randomly drawn):**

  | Signal | Coverage | Notes |
  |---|---|---|
  | A · PDF outline/bookmarks | 1/13 (8%) | `pypdf.PdfReader.outline` — already a dependency |
  | B · Parseable table-of-contents page | 3/13 (23%) | trailing ascending page numbers + dot leaders → title→page directly |
  | C · Body numbering pattern | 6/13 (46%) | `1.1`, `一、`, `第三章` … |
  | D · No usable structure | 3/13 (23%) | must degrade to today's behaviour, unchanged |

- **Signal priority is *not* the obvious one — this is the main finding:** **outline > TOC page > numbering pattern > font size.** Font size, the technique every tutorial reaches for first, **measured near-zero on this corpus**: 3 of 4 inspected documents had 0.0–0.2% of characters larger than body size, i.e. headings share the body font and differ only by weight or position. Demote font metrics to a *bonus* signal, never a precondition. Also note that 4 PDFs carried an outline but **3 of them held a single node** — presence of an outline is not evidence of a usable tree; count the nodes.
- **Phases:**
  - **Phase 0 — feasibility measurement, no code change.** A throwaway script over *real customer-shaped* documents answering two questions: (a) what fraction carry a usable structure signal (rerun the table above on the real corpus), and (b) what fraction of today's chunks actually straddle a section boundary. **If (b) is under ~10%, stop here** — the payoff is not there. Both numbers are structural facts checkable by eye, so **Phase 0 and Phase 1 do not depend on `Q1-3`'s eval set**, unlike Phase 2.
  - **Phase 1 — detection only; no chunking change, no re-index.** `detect_headings(path) -> list[Heading] | None` with the cascade above; each layer must fail independently and fall through, and the `D` case must leave behaviour bit-identical. Feed the result **only** into the `A6a` diagnostics panel ("detected N sections, max depth M") so detection accuracy can be validated against real files **without paying the re-index cost**. Two known traps: a detected TOC page must be flagged *not to index* (or it pollutes retrieval with its own entries), and consecutive TOC pages must be merged. Heading depth is derivable from the numbering itself (`5.2.1.1` → 4; `三、` → 1; `51.` → 2); the zh-TW convention (`壹、` → `一、` → `（一）` → `1.` → `（1）`) is a published standard, so the patterns target a spec rather than guesswork. `ExtractionResult.details` is already documented as "free-form so a new format adds signals **without a schema change**" — the landing spot exists.
  - **Phase 2 — boundaries + ancestor path (requires re-index).** Only after Phase 1 shows detection is accurate enough: `chunk_sections` respects heading boundaries, `heading_path` lands in the chunk row, Chroma metadata, and the citation label. Touches `citation_payload`, the UI citation rendering, and eval `expected_evidence` matching — plan those together.
- **Per-source structure flag (decided 2026-08-22):** record on each source whether a usable structure signal was found, and **which tier produced it** (`outline` / `toc_page` / `numbering` / `none`). It belongs in the `A6a` diagnostics payload (`sources.diagnostics_json`) — the panel already exists and this is exactly the kind of extractor-known fact it was built for. Its job is **index-time gating**: a `none` source must not spend LLM tokens on `Q1-7`'s section summaries. It is *not* needed as a query-time filter — a source with no detected nodes contributes nothing to the section arm automatically.
- **Known corpus risk:** the target deployment's documents are research reports **plus vendor-authored documents of unpredictable structure**. The `none` tier is therefore expected to be **larger in the real corpus than the 23% measured above**, and mixed-structure notebooks (some sources structured, some not) will be the norm rather than the exception. Two consequences: (1) the `none` path is the *primary* path, not an edge case — it must be bit-identical to today's behaviour; (2) mixing creates a retrieval-fairness problem that `Q1-7` must solve, not this item.
- **Deliberately not doing — external structure libraries.** Surveyed 2026-08-22: **PyMuPDF** is the most mature and fastest but is **AGPL**, a licensing risk for customer deployments — excluded on that basis alone. **Docling / Marker / MinerU / unstructured** are accurate but are ML layout pipelines requiring model downloads and inference, which contradicts this project's lean-dependency rule. **libpdf** targets exactly this problem but is still 0.0.1. **GROBID** — **excluded (decided 2026-08-22).** It is only strong on genuine academic papers; the customer corpus is research reports and vendor-authored documents with unpredictable structure, so its narrow assumption does not hold, and it would add a Java service for a subset of a subset. Conclusion: no drop-in lightweight library exists; the cascade above is ~200–300 lines with **zero new dependencies**.
- **Unlocks:** [`QUALITY.md`](QUALITY.md) `Q1-7` (section summaries as a third retrieval arm).
...
- **Chunking:** one chunk per Q&A row, unless the answer alone exceeds the budget — then it splits into parts that each repeat the preamble and the question (`sheet "FAQ" row 12 part 2/3`). Records use token-aware adaptive packing against `[spreadsheet].embed_token_budget`, and a single over-budget row splits into column-group parts that each repeat the identifier column (`sheet "X" row 7 part 2/3`); a single cell larger than the budget — including column 0 — is split in place with its column name repeated, rather than emitted whole. Spreadsheet extraction returns `ExtractionResult.pre_chunked=True` so `chunk_sections` does **not** re-pack rows — sentence packing across rows would glue unrelated records together and destroy the `sheet "X" row N` citation labels.
...
- **Guardrails:** preserve per-user/per-notebook authorization on every query; cap result rows; audit query specs without logging cell contents; add a routing-accuracy eval type before trusting the router (the current Eval Workbench has no table-routing accuracy metric or result type).
...
#### [ ] M3 · Lock transitive dependencies with a compiled lock file (reviewed 2026-09-12, deliberately deferred)
- **Current state:** `requirements.txt` pins the **17 direct** dependencies; CI installs **108 packages**. The other 91 are transitive (`chromadb` alone pulls opentelemetry, grpcio, onnxruntime and the kubernetes client) and carry no version constraint at all, so two installs a week apart can differ with no change in the repo.
- **What a lock file would add:** `pip-compile` (pip-tools) splits intent from resolution — a hand-written `requirements.in`, plus a generated file pinning all 108 with `# via <requester>` provenance comments, optionally `--generate-hashes` for supply-chain verification at install time. `pip-sync` then makes a venv match it exactly, *removing* stray packages an ad-hoc `pip install` left behind. `uv pip compile` is the faster modern equivalent of the same format.
- **Why deferred:** none of the three symptoms has appeared — no "green locally, red on CI with an empty diff" incident (nothing of the sort in [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md)), no unreproducible history that mattered, no upstream breakage. The CI venv cache added in #135 already gives most of the *reproducibility* benefit in practice: one `requirements*.txt` hash maps to one frozen set of 108 packages until Dependabot changes it. It is a poor-man's lock file — not version-controlled and not auditable, but enough at this size. The real cost is not running `pip-compile` once; it is the ongoing change to `setup.sh`, the Dockerfile, `docs/DEVELOPMENT.md`, and how Dependabot is configured to update a `.in`/lock pair.
- **Restart condition:** the first "green locally, red on CI, empty diff" incident; a customer deployment asking for an auditable dependency list or an SBOM; a second long-term developer whose venv starts drifting from yours; or adopting `uv` for unrelated reasons, which makes `uv pip compile` nearly free.
- **Priority:** low — condition-triggered, not scheduled.
