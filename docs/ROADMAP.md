...
- **Foundation landed (2026-06-19):** `app/i18n.py` catalog + `t()` Jinja global + `window.I18N` (base.html) + `[ui].language` config (`NOTEBOOKLM_UI_LANGUAGE`, default `zh-TW`, en deferred). Migrated so far: primary nav + the chat "thinking" string (proof), and the **audit page** fully. Remaining high-churn surfaces (chat states, Studio tool labels, server errors, export headings) + the Eval workbench cluster are the next slices — tracked in [`docs/UX_REVIEW.md`](UX_REVIEW.md) (i18n Phase 1 / 2b).
