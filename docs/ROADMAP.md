...
- **Recommended next implementation round:** do **E1d**. Add sanitized profile export (settings + aggregate metrics, no source text) plus an explicit full internal report export (questions, expected evidence, failures) gated by admin action. Defer LLM-generated questions, answer judging, and index-affecting parameter application to E1e/later.
