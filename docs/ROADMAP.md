...
- **Shipped mitigation (v0.8.1):** `vector_index_state.write_seq` is bumped on every upsert/delete/clear, and `collection()` clears chromadb's cached `System` and reopens the client when it moved. This is correct but reactive: it pays a client reopen on the first read after every foreign write, so a busy ingest makes the query path reopen repeatedly.
