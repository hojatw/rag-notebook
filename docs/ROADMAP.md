...
- **Implementation plan:** [`O0_DIMENSION_RESET_PLAN.md`](O0_DIMENSION_RESET_PLAN.md) — phased breakdown, test matrix, and the design decisions. **D2 (stale-source status) is ruled and implemented**; concurrent-ingest handling, UI placement, and script retirement still await a ruling.
- **Phase A + B landed:** `reset_collection()` replaces the collection object and bumps `vector_index_state.generation`, which invalidates every other process's cached handle; `app/index_migration.py` classifies sources by stored-vector width; `sync_from_sqlite` skips dimension-mismatched chunks (reporting `skipped_dimension`), and old-width sources move to `stale_embedding` — excluded from every `status = 'indexed'` query and recovered through Reindex. Remaining: the write barrier + admin migration flow (Phase C), split-worker coverage and warning removal (Phase D).
- **Permanent-fix acceptance criteria:**
  ...
  2. **[partial]** App and dedicated worker cannot keep stale cached collection handles after reset (**done** via the generation counter); concurrent ingest is blocked, drained, or safely retried (**Phase C** — `reset_collection()` currently documents this as a caller precondition).
