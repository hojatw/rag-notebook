...
- **Foundation landed (2026-06-19):** `app/i18n.py` catalog + `t()` Jinja global + `window.I18N` (base.html) + `[ui].language` config (`NOTEBOOKLM_UI_LANGUAGE`, default `zh-TW`, en deferred). Migrated so far: primary nav, the **audit page** (fully), and the **Studio/chat surfaces** — chat empty-state, Studio tools + suggestions, tool config panels, app.js answer-flow strings, the chat abstain message, and Markdown export headings. Remaining: server-side error copy (`friendly_error_message` + partial inline errors), upload-file JS strings, settings provider hints, and the Eval workbench cluster — tracked in [`docs/UX_REVIEW.md`](UX_REVIEW.md) (i18n Phase 1b / 2b).
