...
| …`chat.capabilities.sampling_params.status` / `max_tokens_field.field` / `reasoning_effort.status` + `supported_values` | `llm._probe_sampling_params` | `_settings_diag_chat.html`, **and `chat_sampling_support` → `build_chat_request` on every LLM call** | status: `succeeded` · `failed` · `not_tested` · `skipped`; field: `max_tokens` \| `max_completion_tokens`; effort values: `none` · `minimal` · `low` · `medium` · `high` · `xhigh` · `max`. Auto always probes `low`/`medium`; fixed mode also probes only its selected value. Runtime accepts effort only when its status succeeded, the exact value was positively probed, and `chat.settings_fingerprint` still matches `llm.chat_settings_fingerprint` — the **chat connection alone** | `test_settings_fixed_reasoning_effort_round_trips_real_probe_contract` |
| …`embedding.max_input_tokens` / `window_status` / `window_bound` | `llm.probe_embedding_window` → `settings.compact_diagnostic_result` → `store_llm_diagnostic` | `_settings_diag_embedding.html`, **and `llm.embedding_window_budget` → `embed_texts` on every query and every ingest batch** | positive int tokens, or `null` when unmeasured; `window_status` uses `llm.DIAGNOSTIC_STATUS_*`; `window_bound` is `exact` (read out of the endpoint's rejection) \| `at_least` (the oversized probe was accepted). Runtime applies it only while `embedding.settings_fingerprint` matches `llm.embedding_settings_fingerprint` — the **embedding connection alone**, resolved through `embedding_settings`, so a chat-side change never discards it — otherwise it falls back to `[diagnostics].embedding_token_budget` | `test_the_probed_embedding_window_survives_storage_and_reaches_the_runtime` |
| …`chat.capabilities.context_window.status` + `max_input_tokens` + `bound` | `llm._probe_chat_window` | `_settings_diag_chat.html`, **and `chat_window_budget` → `fit_rewrite_inputs` + the `build_chat_request` backstop** | status: `succeeded` · `inconclusive` · `not_tested`; bound: `exact` (read out of the rejection) \| `at_least` (the probe was accepted). **Only `exact` becomes a budget** — a floor bounds nothing useful for a chat prompt, which can legitimately exceed the probe. 0 (unknown) leaves every guard off, so an unprobed deployment behaves exactly as before | `test_only_an_exact_measurement_becomes_a_budget` |
| …`chat.capabilities.structured_output.status` + `shape` | `llm._probe_structured_output` | `_settings_diag_chat.html`, **and `structured_output_shape` → `build_chat_request` on every JSON-returning call**, plus `llm.capability_snapshot` | status: `succeeded` · `failed` · `not_tested`; shape: `response_format` \| `guided_json` \| `''`. `succeeded` requires the endpoint to both accept the request **and** return parseable JSON — an endpoint that accepts it and answers prose is `failed`, because acceptance is not a guarantee. Runtime constrains output only when status succeeded, the shape is one it sends, `chat.settings_fingerprint` matches (chat connection alone), **and** `llm_settings.structured_output_enabled` is on | `test_an_endpoint_that_ignores_the_constraint_is_not_recorded_as_supporting_it`, `test_structured_output_fails_closed` |
| `eval_runs.llm_snapshot_json` | `llm.capability_snapshot` → `evals` run creation | `admin_eval_compare.html` via `evals.LLM_SNAPSHOT_LABELS` | `chat_model`, `embedding_model`, `structured_output`, `temperature_sent`, `max_tokens_field`, `reasoning_effort_mode`, `embedding_window_tokens` — model names and capability verdicts only; never keys, base URLs or prompts. `{}` on runs created before the column existed, which the compare view names rather than rendering as "nothing differed" | `test_an_eval_run_records_the_llm_settings_it_ran_under` |
...
| `error` | TEXT DEFAULT `''` | failure message (truncated to 500 chars); also carries the `stale_embedding` explanation, which the source row renders. For an HTTP failure this now includes the provider's own response body (bounded by `[llm_retry].error_body_chars`) — the sentence explaining *why*, e.g. an embedding endpoint naming the token limit the input exceeded, which previously existed only in that service's own log |
| `summary` / `summary_at` | TEXT DEFAULT `''` | per-source TL;DR generated after indexing |
| `structured_output_enabled` | INTEGER NOT NULL DEFAULT 0 | O5b admin opt-in for schema-constrained output. Deliberately **not** in `llm_settings_fingerprint`: that identifies what the probe measured, and flipping a toggle does not change it |
...
| `write_seq` | INTEGER NOT NULL DEFAULT 0 | incremented by `bump_write_seq()` after **every** upsert / delete / clear; compared against each process's cached *client*. `generation` cannot do this job — it means "the collection object was replaced" and only the O0 migration bumps it, so ordinary writes were invisible across processes |
...
**Why there are two counters.** A `generation` mismatch means the collection object was replaced, and re-fetching the handle is enough. A `write_seq` mismatch means another process wrote records, and re-fetching is **not** enough: chromadb caches one `System` per store path and hands it to every `PersistentClient` built for that path, so the stale in-memory HNSW index survives both a new handle and a new client. `collection()` therefore calls `SharedSystemClient.clear_system_cache()` and drops `_client` on a `write_seq` mismatch. Measured against chromadb 1.5.9; `tests/test_vector_store.py::test_query_sees_records_written_by_another_process` pins it with a real subprocess (an in-process module reload shares the cached `System`, so it cannot observe this). See `docs/TROUBLESHOOTING.md`.
...
| `llm_snapshot_json` | TEXT NOT NULL DEFAULT `'{}'` | O5c: the LLM capability flags in effect at run creation. Without it two runs differing only on the LLM side are indistinguishable in the record, so a metric difference between them cannot be attributed — the gap that made an A/B of structured output impossible |
