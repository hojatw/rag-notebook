...
這是概念驗證，不是可直接用於正式環境的服務。設定真正的 `NOTEBOOKLM_SECRET` 之後，它適合本機實驗與小型單機部署，但尚未針對直接暴露在公開網際網路的情境完整強化。請參考[已知後續事項](#已知後續事項)。
...
- **每個 notebook 都有三欄式工作區**：
  ...
  - **Chat**（中間）：有來源根據的對話，包含串流回答、對話切換器（每列可刪除、可**匯出 Markdown**）、Markdown 轉譯回答（每則回答可一鍵**複製**），以及行內 `[1]` `[2]` 引用 chip，點擊會打開來源預覽抽屜並捲動、高亮被引用的那個分塊。提問停留在原頁並顯示逐步狀態；每次成功回答後會建議 2–3 個**追問問題 chip**（延遲生成、逐訊息快取）。空狀態會顯示**建議問題**——由 LLM 根據來源一鍵產生的開場問題，來源完成索引後自動重新整理（24 小時快取）。Enter 送出、Shift+Enter 換行、中文選字 Enter 不會誤送。介面為繁體中文。
  - **Studio**（右側）：NotebookLM 風格工作區，分為環境脈絡、工具啟動器、產出架三部分（Studio IA 重構，見 `ROADMAP.md` U16）。
      ...
      - *工具*：磚格啟動器,每個磚會在 preview-modal 中開啟設定、執行,並把結果連同**手動「存成筆記」**按鈕一起顯示(由使用者決定哪些落到產出架,不自動存)：
          - *來源比較*：選擇 2 個以上已索引來源 → Shared / Distinct / Contradictions 的 Markdown 報告。
...
- **AI governance telemetry backend**：`llm_usage_events` 會記錄檢索、回答生成、Studio 工具、摘要、追問、eval authoring/runs 等 LLM/embedding 呼叫的精簡 usage；`ai_safety_events` 會記錄本地安全規則對選定使用者輸入的發現（長度、不可見/控制字元、疑似 secrets、prompt-injection 類語句），只保存 redacted review signals。Provider token usage 會優先使用；governance metadata 不保存 raw prompts、來源文字、模型輸出、retrieved snippets 或 API keys。
- **多使用者**，包含雜湊密碼與嚴格的每使用者/每 notebook 隔離。管理員可在 `/admin/users` 管理使用者帳號；任何已登入使用者都可在 `/account` 修改自己的密碼。
- **CSRF 保護**：unsafe routes 使用 signed double-submit token；server-rendered forms 會包含 hidden token，HTMX / streaming chat requests 會用 header 傳同一個 token。
...
GET  /search                                              搜尋 notebooks、sources、conversations 與 notes
...
GET  /notebooks/{id}/_chat-empty                          HTMX swap：索引狀態改變後刷新聊天空狀態

POST /notebooks/{id}/chat/new                             新對話
POST /notebooks/{id}/chat/ask                             提問（HTMX 回傳訊息 partial；無 JS 則 303）
POST /notebooks/{id}/chat/ask-stream                      以串流事件提問並回傳最終訊息
POST /notebooks/{id}/chat/{cid}/rename                    重新命名對話
...
GET  /notebooks/{id}/notes/{note_id}/export               下載單筆筆記 Markdown
...
GET  /admin/evals                                         管理員 Eval 工作台：active profile、eval sets、run history
GET  /admin/evals/help                                    產品內 retrieval profile 調參指南
POST /admin/evals/sets                                    為既有 notebook 建立 eval set
POST /admin/evals/sets/{eval_set_id}/delete               刪除 eval set 與相關 items/runs/results
GET  /admin/evals/sets/{eval_set_id}                      eval set 詳情、手動題目建立、run list
POST /admin/evals/sets/{eval_set_id}/generate             從 indexed chunks 產生 draft 題目候選
POST /admin/evals/sets/{eval_set_id}/generate/llm         從選定來源產生 LLM-assisted draft 題目候選
POST /admin/evals/sets/{eval_set_id}/items                新增 approved 手動 retrieval-eval 題目
POST /admin/evals/sets/{eval_set_id}/items/{item_id}/approve approve draft 題目
POST /admin/evals/sets/{eval_set_id}/items/{item_id}/delete 刪除 eval 題目
POST /admin/evals/sets/{eval_set_id}/run                  排入 retrieval-only eval run（可選 profile_id）
...
app/governance.py      AI usage/safety telemetry normalization + sanitized governance recorders。
...
app/templates/
  ...
  _tool_panel.html     載入 preview-modal 的單一工具設定面板（compare/minutes/study_guide/faq/timeline/translate）。
  ...
  admin_eval_compare.html Run 比較頁（兩個 run 的參數／指標／逐題差異）。
  admin_eval_set.html  Eval set 詳情頁，支援手動題目建立與啟動 run。
  _eval_items_section.html HTMX eval set 題目/approve 表格 partial。
  admin_eval_run.html  Eval run 詳情頁，顯示 profile snapshot 與逐題結果。
  _eval_run_status.html HTMX eval run 進度/metrics partial。
  _eval_run_results.html HTMX eval run 逐題結果 partial。
...
tests/
  ...
  test_governance.py   LLM usage telemetry、safety-rule scanning、sanitized persistence、retry metadata、provider/stream usage recording。
...
效能/擴充性與檢索品質 work item 分別追蹤在 [`docs/PERFORMANCE.md`](docs/PERFORMANCE.md) 與 [`docs/QUALITY.md`](docs/QUALITY.md)；UX 改善與新的 AI 功能追蹤在 [`docs/ROADMAP.md`](docs/ROADMAP.md)。仍待處理的重點項目：

- 沒有 offline embedding fallback - 接受上傳前必須先設定 embedding model。
- UI strings 仍 hardcoded zh-TW；i18n foundation 與完整抽取已拆成 `ROADMAP.md` U15a/U15b。
