...
## Studio expansion round (landed 2026-05-17)
...
| Notebook briefing | New `notebooks.briefing` + `notebooks.briefing_at` columns. Cache helper `_cached_briefing(notebook)` mirrors `_cached_suggestions` (24 h TTL). `app/llm.py:generate_briefing` synthesises one paragraph (80–110 words) from per-source summaries (temperature 0.4). Routes: `GET /notebooks/{nid}/_briefing` (HTMX swap on `sources-changed`), `POST /notebooks/{nid}/briefing` (returns cache by default; pass `?force=1` to bypass — that's how the *Regenerate* button works). [`_briefing.html`](app/templates/_briefing.html) empty-state element carries `hx-trigger="load delay:200ms"` so first-time viewers see a briefing without clicking anything. Body renders inside `<details class="briefing-details">` with a one-line preview when collapsed. |
| Source comparison | Interactive: collapsed `<details>` in the Studio pane with checkbox list of indexed sources + optional focus input. `GET /notebooks/{nid}/_compare` re-renders the section on `sources-changed` so the checkbox list stays in sync after upload/delete. `POST /notebooks/{nid}/compare` accepts `source_ids[]` + `focus`, calls `app/llm.py:compare_sources` (Shared / Distinct / Contradictions Markdown structure, temperature 0.3), returns [`_compare_result.html`](app/templates/_compare_result.html) into `#compare-result`. Result fragment includes a *Save to notes* form posting to the new `POST /notebooks/{nid}/notes/add` (raw title + content; mirrors `notes/pin` plumbing). |
...
| Language directives | All three prompts (`SOURCE_SUMMARY_PROMPT` / `NOTEBOOK_BRIEFING_PROMPT` / `SOURCE_COMPARE_PROMPT`) include an explicit per-language matrix (繁中 / 簡中 / 日 / 英) with `Do NOT translate` to prevent the "Chinese source → English summary" drift the weaker `Reply in the same language` instruction allowed through. |
