...
- Read `docs/SSO_DEPLOYMENT.zh-TW.md` (operator guide) when deploying or testing enterprise SSO — reverse-proxy/OIDC setup examples, config snippets, security-contract checklist, and the auth test plan.
...
全套 527 個測試約 30 秒。除錯單一測試時加 `-n0` 關掉並行，否則 `-s` 的輸出與
...
- **Do not hardcode which parameters a chat model accepts, and do not infer it from the model name.** There is no capability endpoint (`GET /v1/models` returns none, and openai-python#3073 is still open), and name-prefix matching breaks on every release. `/settings` → *Test chat model* probes it once and stores the answer; `build_chat_request` reads it back via `chat_sampling_support()`. GPT-5-class reasoning models reject `temperature` and renamed `max_tokens` to `max_completion_tokens` — since **every** LLM call funnels through `build_chat_request`, getting this wrong takes the whole app down rather than degrading it.
- **Every chat call must pass its `call_type`**, which selects the output cap from `[max_tokens]` in `app/config.py`. A `call_type` with no entry there silently falls back to `default`; `tests/test_llm.py` pins the two lists together, so add the field when you add a call site.
