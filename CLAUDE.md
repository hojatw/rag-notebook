...
- `app/domain_policy.py` — E2 notebook domain-hint and answer-policy validation, owner-scoped CRUD helpers, query-time matching/expansion, immutable Eval snapshots, canonical validation, and content-free export/audit summaries. Hints and policy are untrusted guidance, never source evidence.
...
- `app/i18n.py` — the UI message catalog (`t()` / `window.I18N`). New or changed user-facing copy routes through here; `docs/I18N.md` records the remaining legacy inline zh-TW exceptions.
