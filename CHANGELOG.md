...
- **既有的 `.vtt` / `.srt` 來源需要重新索引**才會拿到發言者與時間碼。不重新索引不會
  壞，只是那些來源的引用位置仍然只顯示 `transcript`、發言者仍然是缺的。
- **既有來源的摘要不會自動更新。** 摘要只在（重新）攝取時產生，所以取樣改善只對之後
  新增或重新索引的來源生效。這是已知的限制，不是遺漏——一條「只重算摘要」的路徑仍在
  `QUALITY.md` `Q1-8` 待辦中。

### 新增

- **會議整理改為讀完整份逐字稿，並顯示進度**（`T1a` 第二步）：上一版只做到「告訴你沒讀完」，
  這一版是真的讀完。逐字稿會依對話模型的輸入上限切成數段，每段各自擷取決議與行動項目，
  最後合併成一份會議記錄。兩小時的會議約 2 段、四小時約 4 段，加上合併共 3–5 次模型呼叫；
  **只有一段的短會議完全走原本的單次呼叫路徑**，成本與輸出都不變。因為多段會跑比較久，
  工具視窗現在會顯示「正在整理第 2 / 4 段（transcript 00:15:00 – 00:29:00）…」，區間就是
  上一項做出來的時間碼。整份讀完後，先前的涵蓋率但書只在超長錄音（超過 8 段，約八小時）
  才會出現。判定「這不是會議記錄」時會在第一段就停下，不會把剩下的段數跑完。
- **逐字稿保留發言者與時間碼**（`T1c`＋`T1d` 逐字稿半邊）：WebVTT 的 `<v 王小明>`
  原本會和其他標籤一起被剝掉，**發言者在攝取當下就消失了**，行動項目的負責人因此
  無從判斷。現在會先改寫成 `王小明：` 前綴再剝其餘標籤。同時每一段字幕各自成為一個
  `transcript 00:12:30` 區段，引用位置因此變成它實際涵蓋的時間區間
  （`transcript 00:12:30 – transcript 00:15:10`），而不是每一段都只寫 `transcript`
  ——後者在匯出的交付物裡等於沒有出處。時間碼無法解析的檔案會退回原本的單一區段並在
  診斷面板標示，不會因此掉字。來源預覽的診斷面板新增一行：幾段字幕、幾段有時間碼、
  幾處發言者標記。
- **回答 prompt 有了輸入上限保護**（`O6`）：先前只有 query rewrite 有主動保護，
  回答路徑**只會在超過之後記一行警告**——而它的輸入才是兩者中比較大的那個。超過時
  端點直接拒絕，每個呼叫端都轉成降級結果。現在會依照實測到的輸入視窗，從**相關度
  最低的那一端整段捨棄**檢索內容：不切半段（半段仍然會被標上 `[N]`，引用就會指向
  模型沒看過的文字）、至少保留一段、並把捨棄的段數顯示在回答下方的除錯面板。
  沒有實測過視窗的部署完全不受影響。

### 修正

- **摘要與起始問題不再只看文件開頭**（`QUALITY.md` `Q1-8`）：來源摘要讀的是前 12 個
  分塊，也就是開頭幾頁。對會議逐字稿來說那是議程與寒暄，決議都在後面——於是摘要、
  briefing、學習指南、FAQ、時間軸全都在描述「會議怎麼開始」。現在改為保留開頭三段、
  其餘平均分布到文件結尾（**一定會取到最後一段**），送出的分塊數與 prompt 大小不變、
  不增加任何 LLM 呼叫；真的有分散取樣時才會告訴模型「這些不是連續的段落」。
  起始問題的取樣問題更明顯：原本是 `ORDER BY chunks.id DESC LIMIT 24`，也就是
  **最近索引的那一份來源的結尾**，整個筆記本的起始問題都來自一份文件的最後幾頁。
  現在改為每份來源各自分散取樣、再輪流取用，每份已索引的來源都會被代表。

- **會議整理會說出它只讀了逐字稿的多少**（`T1a` 第一步）：會議整理工具一直只送出
  逐字稿的開頭——資料庫查詢取前 100 段，產生時再以 16,000 字為上限截斷——而畫面上
  沒有任何跡象。以中文口語每小時約 1–1.5 萬字估算，兩小時的會議大約有兩到五成的
  內容從未進入整理，使用者卻看到一份外觀完整的會議記錄。現在只要沒讀完，結果區會
  顯示讀了幾段／共幾段與最後讀到的位置，**同一句話也會寫進存下來的筆記**，匯出的
  檔案才不會丟掉這個但書。全部讀完時不顯示任何提示。
  *（這一版只是揭露，不是修好——涵蓋整份逐字稿是 `T1a` 第二步。）*
- **`/admin/index` 顯示向量檢索的健康狀態**（`O3`）：Chroma 查不動時，檢索會靜默退回
  SQLite 後備掃描，回答照常產生、只是變差，畫面與稽核都不會有任何錯誤——2026-09 的
  兩次事故分別持續了約兩小時與兩天才被發現。索引頁現在有一段健康狀態：正常時顯示最後
  一次成功的時間，一旦連續退回就轉為警告並標出連續次數，達到門檻改為紅色警示。下一次
  成功會自動清除。**注意計數只涵蓋當下這個網頁行程、重啟歸零**，頁面上也寫了這件事。
- **評測比較頁會在兩次執行用了不同模型時提出警告**（`E1g`）：LLM 設定差異表早就存在，
  但「換了對話模型」在畫面上和「換了一個參數」長得一模一樣，而後者正是比較功能本身的
  用途。現在只要 `chat_model` 或 `embedding_model` 不同，頁首就會出現警告，說明下方的
  指標差異不能只歸因於參數。結構化輸出等其他設定的變動**刻意不觸發**警告——那些正是
  評測要跑的實驗，對它們示警只會讓人很快忽略所有警告。


## [0.10.0] - 2026-09-18

本版只有一項新功能與一項修正，但**兩項都需要升級後到 `/settings` 重按測試按鈕**才會
生效。修正的是 `0.9.0` 自己引入的一個靜默退化：只要換了對話模型，`0.9.0` 剛測得的
embedding 輸入視窗就會一起被丟掉。新功能則把同一套探測延伸到對話模型本身，讓超長
提問不再是「整串送出去、端點回 400、使用者什麼都拿不到」。

### 升級注意事項

**資料面不需要任何動作**：不必重新索引、不必刪除資料、不必改設定鍵，沒有新欄位。

**但有一項手動動作，不做的話這一版等於沒升**：升級後請到 `/settings` 各按一次
「測試 embedding 模型」與「測試對話模型」。

- 能力探測的有效範圍這一版改成「每條連線各自一個識別碼」，**既有的探測結果會因此
  失效一次**。沒重測不會壞——所有讀取端都是 fail-closed，會退回設定值與預設行為——
  但新的測量值要重測才會回來。
- 對話模型的輸入視窗是這一版才開始測的，**沒按「測試對話模型」就完全量不到**，
  長問題的保護也就不會啟動。

### 新增

- **測得對話模型的輸入視窗上限，並據此保護 prompt**：先前送進對話模型的內容**沒有
  任何一層有上限**——提問長度在路由與前端都沒有限制，`history[-6:]` 限制的是歷史
  「則數」而非每則長度。真的塞爆時端點回 400，每個呼叫端都轉成降級，而 query rewrite
  的降級結果是「只剩原始問題當查詢」，接著 embedding 也失敗，**使用者什麼都拿不到**。
  現在「測試對話模型」會一併測出視窗上限（沿用測 embedding 視窗那套：刻意送一次過長
  輸入，直接從拒絕訊息讀出數字）。測得之後，query rewrite 會先丟掉最舊的歷史訊息，
  仍然不夠才動問題本身——而且**保留頭尾而非只留前段**，因為中文商務提問經常把背景
  寫在前面、真正的問句放在最後一句。另外所有呼叫都會在估算超過視窗時記錄
  `chat_input_over_window`。
  **沒測過就完全不做任何限制**，行為與先前一模一樣：用沒有量測過的數字去截斷 prompt，
  等於讓每個部署為了防一個尾端風險而付出檢索品質，比現狀更糟。
  保護目前只涵蓋 query rewrite；答案生成那條路徑（歷史 + 檢索片段 + 問題）輸入更大，
  但「什麼可以砍」的判斷不同（檢索證據才是主體），這一版只給它上面那條紀錄，
  沒有主動保護。

### 修正

- **換 chat 模型會把 embedding 的測量結果一起丟掉**：能力探測的有效範圍先前由一個
  涵蓋兩條連線的識別碼決定，所以只要動到對話模型、base URL、temperature 或 timeout，
  **即使 embedding 連線一個字都沒改**，0.9.0 測得的輸入視窗也會被作廢、悄悄退回設定值
  512。長問題會被截得更短，沒有錯誤、沒有任何一行 log 說「測量值不見了」。
  對話與向量各自使用不同主機、需要在兩者之間切換的部署（例如主力機維修期間暫時改用
  備援模型）一定會踩到。
  現在改為每條連線各自一個識別碼，且以**解析後的連線**計算——`embedding_settings`
  在未設定獨立欄位時會沿用共用的 chat 欄位，只看原始欄位會漏掉「改了 chat base URL
  其實也改了 embedding 端點」這種情形。反方向的耦合（改 embedding 模型會作廢 chat 的
  能力探測）是既有問題，一併修正。

### 依賴

- `pypdf` 6.16.2 → 6.18.1（#137）。6.18.0 與 6.18.1 各含數項解析器加固
  （限制 indirect object token 長度、限制字型 `/Widths` 筆數、限制 `parse_bfchar`
  的 token 長度、收緊 FlateDecode 復原），上游 release note **未附 GHSA 編號**，
  因此不另外進 `docs/SECURITY.md` 的判定表。本專案只在 pdfplumber 抽不出內容時
  才改用 pypdf，這些加固都落在那條退回路徑上。
  6.18.0 的 `DEP: Rework configuration value handling` 不影響本專案——
  `app/ingest.py` 只用 `PdfReader(str(path))`，沒有覆寫任何 pypdf 限制值。
  只需重建 image 或重跑 `./setup.sh`，不必重新索引。
- `joserfc` 1.7.4 → 1.7.5（#138）。


## [0.9.0] - 2026-09-17

本版修正四個各自獨立的檢索與 LLM 問題，其中三個**不會顯示任何錯誤**——服務照常回答，
只是答案悄悄變差；另一個會讓長問題完全得不到回應。另新增三項端點能力探測，取代原本
靠人記得調整的設定。

### 升級注意事項

**資料面不需要任何動作**：不必重新索引、不必刪除資料、不必改設定鍵。三個新欄位
（`vector_index_state.write_seq`、`eval_runs.llm_snapshot_json`、
`llm_settings.structured_output_enabled`）都由啟動時的既有 migration 自動補上。

**但有一項手動動作，不做的話新的探測能力不會生效**：升級後請到 `/settings` 各按一次
「測試 embedding 模型」與「測試對話模型」。

- 沒測 embedding：送出前的修剪繼續沿用設定值 512（e5 的視窗）。使用視窗更大的模型
  時，長問題會被截短，影響檢索品質但不會失敗。
- 沒測對話模型：「限定 JSON 輸出格式」無法開啟（探測沒過就不准開，這是刻意的）。
- **日後每次更換模型都要重測一次**——探測結果綁在連線設定的 fingerprint 上，
  設定改過之後舊結果會被忽略。

**「限定 JSON 輸出格式」預設關閉，建議先 A/B 再開**：對 reasoning 模型而言，限制輸出
格式有可能反而降低內容品質。請用評測工作台跑同一組題目、開關各一次，比較頁的
「LLM 設定差異」會顯示這次差在哪裡，再依 Recall/MRR 決定。

### 新增

- **可選擇讓模型只能產生符合格式的 JSON（O5b，預設關閉）**：`/settings` 的「測試對話模型」
  現在會多探測一件事——端點支不支援限定輸出格式（`response_format: json_schema`，或 vLLM
  舊版的 `guided_json`）。探測成功後，管理員可以在設定頁勾選開啟，rerank、查詢改寫、
  追問建議與起始問題這四種會解析 JSON 的呼叫就會帶上 schema，**從取樣層面讓格式錯誤
  不可能發生**，而不是事後補救。
  **預設關閉，而且探測沒過就不准開啟**（與 reasoning effort 固定模式同一個規矩）。
  沒探測過、探測失敗、連線設定改過——全部維持現狀，送出不帶限制的請求。
  探測的判定不只看「端點收不收」：**還要求回傳的內容真的能解析成 JSON**。一個收下請求
  卻照樣回散文的端點會被判為不支援——收下不等於遵守，而誤判成支援會讓執行期信任一個
  並不存在的保證。
  ⚠️ **開啟前請先做 A/B**：對 reasoning 模型而言，限制輸出格式有可能反而降低品質。用
  評測工作台跑同一組題目、開關各一次，比較 Recall/MRR 再決定。#141 的容錯解析仍然保留，
  兩者是互補而非取代。
- **評測執行會記錄當時的 LLM 設定（O5c）**：`eval_runs` 先前只凍結檢索參數與 E2 領域設定，
  **沒有凍結 LLM 這一側**。於是兩次只差在對話模型或某個能力開關的執行，在紀錄上完全
  一樣——數字有差也歸因不到任何東西。現在會一併記錄對話／embedding 模型、限定輸出格式、
  temperature 是否送出、輸出上限欄位、reasoning effort policy 與 embedding 輸入視窗，
  並在比較頁新增「LLM 設定差異」區塊，有變動的列會標示出來。
  只記錄模型名稱與能力判定，**不含金鑰、base URL 或任何 prompt 內容**。
  此功能加入前建立的執行沒有這份紀錄，比較頁會明說「沒有記錄」，而不是顯示成「沒有差異」。
- **自動測得 embedding 模型的輸入視窗上限（O5a）**：`/settings` 的「測試 embedding 模型」
  現在會多問一件事——這個模型最多吃多長的輸入——並顯示在診斷卡片上。先前這個值只能從
  `[diagnostics] embedding_token_budget` 猜（預設 512，即 e5 的視窗），**換用視窗更大的
  模型之後如果沒人記得調高，長問題會一直被截短**，不會失敗、也不會有任何提示，只是檢索
  品質悄悄變差。
  做法是刻意送一次過長的輸入，直接從端點的拒絕訊息裡讀出上限（`maximum context length
  is N tokens`）——一次請求就有確切數字，不必二分搜尋，也不依賴 token 估算的準確度
  （估算刻意往上取整，用它去搜出來的邊界會偏高，對上限而言方向是錯的）。端點若整批吃下
  就回報「至少 N tokens」，對查詢路徑而言已經足夠。
  **測得的值只有在連線設定沒變時才會套用**（比對 fingerprint），沒測過、測不出來、或設定
  改過都會退回設定值，行為與現在完全相同。**換 embedding 模型後請重新測試一次。**
  註：`embedding_token_budget` 仍然會影響試算表的切塊大小，調整它一樣需要重新索引；
  測得的視窗則只影響送出前的修剪，不動已存的 chunk。

### 修正

- **模型輸出的 JSON 只要有一處瑕疵，整批結果就被丟掉**：`rerank` 與 query rewrite 的
  解析先前直接 `json.loads()`，物件之間漏一個逗號、或字串裡有一個沒跳脫的引號，20 個
  候選的排序就全部作廢、退回原始 hybrid 順序。使用者看不到錯誤，只會覺得引用挑得比較
  差（2026-09 一週內三次）。現在嚴格解析失敗時會改走逐物件／逐行的容錯復原，並記錄
  `rerank_scores_salvaged` / `json_strings_salvaged`（含原始錯誤與救回筆數）。
  **格式正確的輸出完全不受影響**，不會走到容錯路徑。
  附帶一提，這三次**都不是**輸出被 `max_tokens` 截斷——截斷的例外訊息是
  `Unterminated string`，而這三次是 `Expecting ',' delimiter`，兩者的處理方式不同，
  對照表在 `docs/TROUBLESHOOTING.md` 第 5 則。
- **問題太長會完全問不到答案**：embedding 請求先前只依「則數」分批，對單則文字長度
  ...
  設定值只是沒測過時的退路——**同版的 O5a 會直接向端點測出真正的上限**（見「新增」），
  所以升級後按一次「測試 embedding 模型」比手動調這個值更準確。
...
- **測試改為預設並行執行**：新增 `pytest.ini`（`-n auto --maxprocesses=8`）與
  `pytest-xdist` 開發依賴，全套測試由單核約 132 秒降到約 30 秒，CI 同步受惠。
  測試內容與覆蓋率完全未動。除錯單一測試時用 `pytest -n0` 關掉並行。

### 效能

- **API 金鑰的解密不再每次重推導金鑰**：加密用的 Fernet 金鑰由 `NOTEBOOKLM_SECRET`
  經 200,000 次疊代的 PBKDF2 推導而來，原本每次呼叫都重算一次，而系統每次讀取 LLM
  設定會解密兩把金鑰（對話與 embedding），出現在對話、提問與匯入等路徑上。改為
  依 secret 快取後，這兩次解密由約 164 毫秒降到 0.06 毫秒（程序啟動後的第一次仍
  需推導一次）。**只有實際有填 API 金鑰的部署會感受到差異**——金鑰留空時原本就
  會直接略過解密。

## [0.8.0] - 2026-09-12

本版新增「回答回饋」功能，**直接升級即可，不需要任何人為動作**：資料表會在啟動時
自動建立，既有回答也能立刻評價。另含一項未經驗證資訊揭露的安全性修正。

### 新增

- **回答回饋（E3a）**：每則 AI 回答下方新增一列評價——**可以直接採用／方向對但不完整／
  不能用**——一次點擊即可送出。選了後兩者時會接著詢問原因（找不到資料、答錯或不完整、
  引用對不上、資料裡沒有卻硬答、明明有資料卻說找不到、太慢或格式問題、其他，可複選、
  可自填）；選「可以直接採用」則互動就結束，不會追問原因，先前勾選的原因也會一併清除。
  送出原因後面板收合並顯示「已記錄：<評價> · <原因>」，讓使用者看得到結果。同一則回答
  再次評價會更新原紀錄，不會產生第二筆。管理員新增「回饋」頁（`/admin/feedback`），
  可依評價、原因、notebook、日期篩選，顯示依評價與依原因的總計，並可就地展開被評價
  回答的全文。
  - **這些數字不是品質指標**：每則回饋來自不同的問題、時間與使用者期待，屬於不受控的
    樣本，不能用來證明參數調整有效；要驗證調整請用固定題集在「評測」頁比較。頁面上也
    直接寫了這句話。
  - **隱私**：回饋存在專屬資料表（`answer_feedback`），保護等級比照 `messages`；稽核
    只記錄「誰在何時給了什麼評價與原因」與自填文字的字數，**不記錄自填內容**。管理員
    開啟回饋頁本身也會寫入稽核。使用者送出前，畫面會註明回饋將提供給系統管理員。
  - 新增可調參數 `[feedback]`：`enabled`、`other_reason_max_chars`、`admin_page_limit`、
    `admin_page_limit_max`。關閉 `enabled` 後聊天畫面不再顯示回饋控制項，既有紀錄仍可查閱。
  - 升級不需任何動作：資料表會在啟動時自動建立，既有回答也能直接評價。
...
- **「設定」頁的 embedding 維度不符警告不再叫管理員「先 Clear 再 Rebuild」**：Chroma
  在第一次寫入時鎖定集合維度，清除不會解鎖、重建也只重播 SQLite 已存的向量，照舊
  文案操作會得到一個仍拒絕新維度的索引。警告改為指向 `/admin/index` 的「更換 embedding
  維度」遷移流程（O0），並說明完成後再儲存設定、重新索引列出的來源。評測「調參指南」頁
  「不能直接套用」卡片同樣改寫：chunk／embedding 類變更需重新索引，維度變更走遷移流程。

## [0.7.1] - 2026-09-11

本版只有依賴更新，其中一項是安全性更新（pypdf），**直接升級即可，不必重新索引**。
重建 Docker image 會得到乾淨的相依環境；本機既有的 `.venv` 重跑 `./setup.sh`
會升級套件，但不會移除已不需要的 `authlib`——它不會被任何程式載入，留著無害，
要完全乾淨可改跑 `./setup.sh --force`。
...
升級注意事項。起因是客戶上傳某個 PDF 後 embedding 端點回 HTTP 400，追下去發現
整條 ingest pipeline 只用字元數當上限，從來沒有一處用 token 數把關。

### 新增

- **`tests/inspect_file_tokens.py`：單一檔案的 embedding token 離線檢查。**
  對指定檔案跑與 ingest worker 完全相同的 `extract_sections` + `chunk_sections`，
  再用模型自己的 tokenizer 數真實 token。不連網、不碰 DB、不碰向量庫，
  可以直接指著客戶的檔案跑，用來回答「這個檔案上傳後會不會因為 chunk 過長而失敗」；
  任何 chunk 超限時 exit code 為 1。既有的 `tests/inspect_e5_chunk_tokens.py`
  掃的是資料庫裡「已索引」的 chunk，剛好看不到最需要診斷的情況——被端點拒絕而
  失敗的來源一筆 chunk 都不會留下。用法見 `docs/DEVELOPMENT.md` 的驗證章節。
...
- **token 估算改為逐字元類別計價**：`estimate_embedding_tokens` 原本只用整個
  ...
  索引失敗。`governance.estimate_tokens` 用的 `cjk_chars_per_token` /
  `latin_chars_per_token` 維持原樣：它只拿得到字元數、看不到文字，本來就無法分類。
...
- **`[spreadsheet].embed_token_budget` 預設 400 → 500**：400 是對著舊估算器訂的，
  ...
  這個 1.04 只適用於這條路徑——前言與 `欄位名 = ` 標籤永遠會稀釋掉病態內容；
  在字元式 chunker 那邊，同樣的內容量到 0.98 倍。別把數字搬到另一條路徑用。
- **LLM／embedding 的 HTTP 錯誤現在會保留供應商的說明**：`_post_json_with_retry`
  ...
  內文長度由新增的 `[llm_retry].error_body_chars` 限制，避免會回吐請求內容的
  供應商把整份文件寫進 log；設為 `0` 可完全關閉。
...
- **必須重新索引全部來源。** 本版改變了 PDF／PPTX／DOCX 的表格文字形狀，
...
- **`[diagnostics]` 新增四個 `tokens_per_*` 參數。** 這是該節唯一一組
  「不只影響顯示」的診斷參數：它們同時是試算表分塊的預算依據，改動會改變
  XLSX／CSV 的 chunk 形狀並需要重新索引。
...
- **仍然沒有硬性的 token 防線。** 本版讓所有已知路徑都有界，但那是靠量測而非
  保證：embedding 呼叫前沒有任何一處量測真實 token。若又遇到索引失敗，先用
  `python -m tests.inspect_file_tokens <檔案>` 確認是不是同一類問題。
  背景記在 `docs/QUALITY.md` 的 Q0-5。
